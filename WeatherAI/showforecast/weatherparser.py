...
def get_forecast_weather(city):
    ...
    data = response.json()
    result=[]
    for stamp, t in zip(data['hourly']['time'], data['hourly']['temperature_180m']):
        stamp = datetime.fromtimestamp(stamp)
        if stamp.hour==0 or stamp.hour==6 or stamp.hour==12 or stamp.hour==18:
            result.append({
                'temp' : t,
                'datetime' : str(stamp)
            })
    return result
#print(pd.DataFrame(get_forecast_weather('Berlin')))